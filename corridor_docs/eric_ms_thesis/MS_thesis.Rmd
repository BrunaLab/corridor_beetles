...
All dung beetles were counted and identified to species using @nemes_illustrated_2015 and @edmonds_taxonomic_2023. Fifteen individuals of each species with adequate captures were dried to equilibrium and weighed for biomass measurements. **[EDIT SENTENCE ABOVE AS FOLLOWS:** Fifteen individuals of each species with adequate captures (was there a numeric cutoff?) were dried at XXX degrees until they reached a stable mass. We weighed each beetle using a BRAND, MODEL microbalance, except for species whose individuals were extremely small. Individual biomass for these species was estimated by weighing batches of N = 5  (i.e., ---------) or N = 3  (i.e., ---------) beetles and then calculating the average biomass per beetle. These values of individual biomass were then used to estimate the total dung beetle biomass -- all species combined and for each individual species - in each block and patch. **]** Voucher specimens for each species will be deposited at the Florida State Collection of Arthropods upon completion of all analyses. 
...
**ADD RESULTS FOR DB BIOMASS ANALYSES**
...
**[Paragraph 7: spatial scale of experimental plots vs. dung detectability]** Finally, it is important to emphasize that while dung beetles are capable of long distance flight and detecting dung at distances of over X meters, the results I observed are lijkely not the result of a mismatch between the spatial scale of the experimental replicates and dung beetle movement. Put another way, some beetles were almost certainly drawn by the dung used in baits from the matrix into the plots. If this were a big effect we'd expect all plots to be similar to the matrix, or at least to each other. the fact that rectangular are lower suggests that...]
