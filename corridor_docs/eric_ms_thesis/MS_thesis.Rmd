...
2) Is there anything about these species that might be interesting in the context of corridors?
  - are any species known to be good fliers/movers?
    ...
    = stanbrook and king 2022 -> tunnelers prefering open habitat and tunnelers also contributing more towards dung removal. 
...
hill numbers are a set of biodiversity metrics designed to quantify biodiversity and richness differently depending on how we want to emphasize abundance while also accounting for differences in sampling sizes and efforts (read more about this Eric)
...
Biodiversity between patch types was compared using Hill numbers (Jost 2006). Hill numbers are....  They are an alternative to...We looked at community composition by increasing magnitudes of diversity components (qD) of 0D (species richness), 1D (Shannon entropy), and 2D (Simpson Diversity). Diversity numbers were calculated in R studio using package hillR and modeled using package lme4. For the purposes of this project richness was modeled using a generalized linear mixed model with a poisson distribution. A qqplot of our data shows that our residuals are normally distributed (Figure 1). 
