...
~~2) Is there anything about these species that might be interesting in the context of corridors? Are any species known to be good fliers/movers? Specialists on deer dung? navigate by starlight?~~
...
- Chao et al. 2014: incorporating abundance into calculating diversity metrics is important for studying functionality in particular since some species that may not be as abundant as others may not contribute to an ecosystem function as much as others. At the same time rare species may be filling a specific niche or we might be concerned about exotics or at risk species. 
...
Species-patch interactions indicated indicated species specific responses to treatments. In matrix plots, the positive effect on abundance was reduced for all other species (e.g., cvig × Matrix $\beta$ = –0.59, p < 0.001). _Ateuchus lecontei_ and _Phanaeus igneus_ showed increased abundance in rectangle patches relative to expectations ($\beta$ = 0.40 and 0.47, respectively; p < 0.05). Random effects indicated moderate variability among blocks (standard deviation of intercept = 0.51).  
...
We used linear mixed-effects models to compare the influence of patch type on both Shannon Diversity and Simpson's indexes, including block as a random effect to account for spatial variation. In both models, the reference level for patch type was Connected.
...
This study advances our understanding of the factors shaping dung beetle community composition in temperate regions of the southeastern United States. In addition, the experimental design enables direct comparisons between populations in continuous matrix habitat and those in both isolated and corridor connected patches. Our main findings emphasized: (1) Dominant species were most abundant in forested matrix, but not equally for all species, some were represented proportionally higher in matrix than in open patch habitat. (2) Patch shape and isolation had less of an influence on species richness which was relatively even on both a patch and block level. (3) Species diversity metrics were also relatively even across patch types however varied widely by block. These results suggest that dung beetle species are fully capable of permeating fragmented landscapes and that habitat type and connectivity shape community compositions, but landscape effects of a larger scale are driving changes in biodiversity. 


## Abundance
 
While habitat type may have an impact on the individual abundances of dung beetles, the arrangement of individuals show little difference as a result of the presence of movement corridors. Past studies have shown similar trends where habitat type and forest regeneration stages are key in the partitioning of dung beetle assemblages[@conover_phenology_2019; @WOS:000386714700011; @arellano_response_2008]. This is further supported by the fact that capture rates were lowest in the rectangular patch, the treatment with the least edge to area ratio, suggesting that dung beetles in the matrix may act as a source population which feeds into open patches through habitat edge. 
 
## Richness
 
We did not detect any patterns of species richness between patch types in our modeling. Yet in comparing raw species counts the sampling plot with the most total species captured was in the matrix while the fewest species sampled were in a rectangular patch. _here look at other srs papers and see whats going on with edges_. 
...
We thank the USDA Forest Service for maintaining experimental landscapes and assisting in getting established at the site. We also thank Thomas Smith for his help in data collection, Sara Escobar-Chena for her help in processing and data entry. 
...
```{r Table bray_patches, echo=FALSE,message = FALSE,warning=FALSE}
...
```{r Table M_rich, echo=FALSE,message = FALSE,warning=FALSE}
...
```{r Table M_shan, echo=FALSE,message = FALSE,warning=FALSE}
...
```{r Table M_simp, echo=FALSE,message = FALSE,warning=FALSE}
