...
1)  Finish adding citations

2)  Stat results: biology first, then stats (e.g., abundance ranged form xxx in rectangular to xxx in winged)

3)  Review other papers to find:

-   those with similar results
-   those with contrary results
-   do this with dung bettles AND other insect taxa

DOWN THE ROAD

1)  **Results:** Did you get any species not captured in other long-leaf/SE USA forest sites?

2)  **Methods:** Find me the best example paragraph you think you read describing the use of hill numbers to estimate diversity/richness (field survey paper)

3)  **Methods:** Find me the best TWO descriptions of using GLMM to compare abundance/diversity/richness at the Corridor Project

4)  **Methods:** Write a short paragraph on how you calculated and compared functional diversity (how you measured it, how you compared it)

5)  **Figure:** biomass of different (R/T/D) functional groups.

6)  **Table:** Results of functional diversity analyses.
...
We thank the USDA Forest Service for maintaining experimental landscapes and assisting in getting established at the site. I also wanted to specifically thank Thomas Smith for his help in data collection, Sara Escobar-Chena for her help in processing and data entry. 
...
*[**EB edit:** break this next sentence into 3 to carry reader though the logic: changes in species diverstiy are important to understand because species are involved in interactions. If we gain or lose species we could gain or lose interactions and resulting ecosystems processes. there is some evidence of this, but... look at the green text notes below]* 
...
By measuring changes in biodiversity and species richness within experimental designs we are able to isolate factors might be contributing to ecological patterns and processes [@fletcher_jr_landscape_2023; @resasco_contribution_2017]. Past studies have endeavored to experimentally measure changes in community compositions as a result of connecting habitats with corridors [@collins_fragmentation_2017; @graham_impact_2022; @tewksbury_corridors_2002] **here fix citations**, yet very few have directly compared matrix and patch populations. **examples of dispersal concepts (drift fence etc** Land use is different from one species to another so it is vital to understand where compositions are distributed and what processes might be driving population differences. 
...
When comparing the overall abundance of beetles (all species combined) across patch types, matrix plots had the highest captures, followed by connected patches, then winged, with the fewest in rectangular patches. Abundances from connected patches were not significantly different from those in the matrix while rectangle and winged patches had significantly fewer than the matrix ($\beta$ = -0.528 and -0.303 respectively, P \< 0.001, Table 4). 

Statistical analysis of abundance focused on the six most abundant species. A generalized linear mixed model identified significant effects for both species ID and patch type on dung beetle abundance (Table 7). The baseline abundance corresponds to the abundance of *Aphodius alloblackburneus* in matrix patches. Compared to this baseline, results were highly variable, emphasizing species specific responses to patch type. Similarly, species showed to have disproportionate responses to patch since interaction terms varied widely (Table 6).

### Beetle Richness and Diversity

Plotting species richness by patch type reveals consistent richness across patch types with some variation between sampling blocks (Figure 5).The number of species per patch varied from N = 8 (rectangle patch in block 8) to N = 13 (matrix patch in block 53N). Modeling the effect of patch type on species richness with block as a random effect determined there was no significant differences among patch types. Comparing treatments using matrix patches as a baseline resulted in no significant differences in connected ($\beta$ = 0.216, p = 0.839; Table 7), rectangle ($\beta$ = -0.548, p = 0.583; Table 7), and winged patches ($\beta$ = -0.095, p = 0.663; Table 7).

Biodiversity was also even among patch types, however metric values were varied by sampling blocks. For Shannon Diversity, the estimated mean in matrix patches was 5.202 (SE = 0.665, t = 7.825). None of the alternative patch types showed statistically significant differences compared to matrix: connected ($\beta$ = –0.085, SE = 0.495, t = 0.17), rectangle ($\beta$ = 0.234, SE = 0.495, t = 0.472), or winged ($\beta$ = –0.251, SE = 0.495, t = –0.506). Similarly, for Simpson's Diversity, the average value in matrix patches was 4.181 (SE = 0.640, t = 6.535). Again, none of the other patch types had significant effects: matrix ($\beta$ = -0.107, SE = 0.411, t = -0.259), rectangle ($\beta$ = 0.059, SE = 0.411, t = 0.143), and winged ($\beta$ = –0.480, SE = 0.411, t = –1.169).
...
\begin{flushleft} 
\textbf{Paragraph 1: }Overview: While there may indeed be effects of patch area and connectivity on dung beetle abundance on community composition, other (landscape-scale) drivers appear to be more important.
\end{flushleft} 
...
\begin{flushleft} 
\textbf{Paragraph 2: }Effects of Edge vs. Connectivity
\end{flushleft} 

In our comparison of dung beetle compositions, connectivity and habitat edge accounted for differences in dung beetle abundances, yet species richness and diversity were even across patch types and in  the matrix. Total beetle counts were consistently highest in the matrix, in comparison abundances is connected patches were not significantly different while those of the winged and rectangular patches were lower. Additionally, we also observed that patch effects were not equally proportional for all species. This suggests that the species captured could be exhibiting habitat preferences between the open patches versus forested matrix. Another potential explanation is that species in the matrix are acting as a source population which feeds into patches with edge acting as a drift fence for directing movement. The latter example seems more likely because flies, which have similar resource dependencies were also found to have a similar interaction ** fried **, and also studies on dung beetles in the tropics found more dramatic differences in dung beetle populations in fragments and matrix [@bustamante-sanchez_dung_2004; @WOS:000180797500010; @WOS:000288810500026].     


\begin{flushleft} 
\textbf{Paragraph 3: }What then are the drivers of dung beetle diversity and abundance?
\end{flushleft} 

Although richness and diversity were the same among treatments, there was notable variability between sampling blocks. Block 8 generally had the lowest species richness and biodiversity while 53n had the highest. While the design of the experimental blocks control for as much as possible, landscape features across the sight are different enough to influence species composition. Several factors may explain this pattern, including soil quality which influences where beetles can reproduce [@arellano_response_2008; @conover_phenology_2019] as well as matrix permeability, canopy cover, and different land-use types (e.g., roadways, water sources)** harvey **[@WOS:000651446900001]. Not only do these impact dung beetle movement, but also these landscape features can affect mammal movement, which in turn limits dung availability for beetles.  

\begin{flushleft} 
\textbf{Paragraph 4: }That said, there were some interspecific differences in abundance. 
\end{flushleft} 

Species specific differences are apparent but do not follow any particular trend. _Aphodius alloblackburneus_ had a disproportionate positive effect to being captured in matrix patches as compared to other species. Beetles from the genus _Aphodius_ do tend to show patterns of habitat specificity[@roslin_distribution_2001], so it is not unexpected that they might show a preference towards either forested matrix or open patches. We did not detect that any species was more positively associated with open patches despite expectations that some species ( eg. _Canthon vigilans_, _Melanocanthon bispinatus_) would prefer open habitat [@conover_phenology_2019; @nealis_habitat_1977]. Altogether this could be another sign pointing towards matrix acting as a source population, and since open habitat was much less dominant in our experimental system beetles could be moving into patches from habitat edge. 


\begin{flushleft} 
\textbf{Paragraph 5:} Our results largely echo/constrast with previous ones from the SRS and elsewhere 
\end{flushleft} 

Despite dung beetles being a well studied model system, most of the current work in fragmented habitats come from tropical climates[@nichols_global_2007], with any work in temperate locations coming from Europe where _Aphodius_ beetles dominate[@roslin_distribution_2001]. Many studies in the tropics have found patterns of habitat preference between open fields and continuous forest similar to what we did in this study[@arellano_response_2008; @WOS:000349242000004]. However, differences in composition were much more distinctly different with some species being exclusive to habitat types unlike we see in this paper. Of the limited work out of the south eastern United States we did recover assemblages similar to other work [@conover_phenology_2019; @nealis_habitat_1977; @young_pines_nodate], and even returned results to another study where differences between treatment types were non-significant while variation on a larger scale was more apparent[@young_dung_2023]. In comparison to other studies from SRS our results were similar to thise  of small mammals which were more likely to be using matrix land *mabry*. We did expect some species to be more dominant in patches with higher edge ratios as was found with ants at SRS [@resasco_landscape_2014], but species were evenly present in all patches. 

\begin{flushleft} 
\textbf{Paragraph 6: }Caveats
\end{flushleft} 
...
- potential effects of seasonality   

- Sampling wasn't designed to look for potential effects of edge proximity, and so we couldn't test for previously observed pattern of generalist/opportunistic species more common near edges.   

- used great bait, but possible some species (eg specialists) not attracted   

\begin{flushleft} 
\textbf{Paragraph 7: }Moving forward
\end{flushleft} 

- tough to compare when most of the work has been done in the tropics; surprisingly few studies of this species-rich and ecolgically vital guild in the SE US. "Despite ample work documenting their ecological importance, there is a suprising lack of research on dung beetle diversity and corridors outside of the tropics.  That said, tropical studies find stark contrast between fragments/pasture. They have been found to use corridors, but it is...."

- experimental approaches to track movement and distinguish beetles already in pacthes vs. those coming from outside. Our preliminary data with MRR indicates individuals do move through corridors. 

\begin{flushleft} 
\textbf{Paragraph 8: } Close with takeaways for conservation and management
\end{flushleft} 
...
-   what do think about corridor design and considerations for fragmented landscapes  

-   practical applications think about the beetles  


\begin{flushleft} 
\textbf{A few things to consider incorporating}
\end{flushleft} 

- ellen wind paper/ connectivity
...
The total number of dung beetle species captured N = `r total_spp` and the total individuals N = `r total_N` are similar to those in studies conducted in similar habitats and regions of the US [@conover_phenology_2019; @nealis_habitat_1977; @stanbrook_dung_2022].
...
```{r echo=FALSE, fig.align='center', fig.cap="Experimental block with the different patches and distance between them.", message=FALSE, warning=FALSE, label=srs-map, out.width="40%"}

knitr::include_graphics("images/srs-map.png")

```

```{r, label = patches, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Experimental block with the different patches and distance between them.", out.width="50%", fig.align='center'}

knitr::include_graphics("images/patches-pic.png")

```

```{r, label = pitfalls, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Experimental block with the different patches and distance between them.", out.width="50%", fig.align='center'}
...
\newpage
```{r, label = patch, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Dung beetle abundance in three different --- and the forest matrix surrounding patches.", out.width="50%", fig.align='center'}

knitr::include_graphics("images/sp_abund.png")

```

```{r, label = abundance, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Dung beetle abundance in three different --- and the forest matrix surrounding patches.", out.width="50%", fig.align='center'}

knitr::include_graphics("images/sp_abund_patch.png")

```

```{r, label = richness, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Dung beetle secies richness in three different --- and the forest matrix surrounding patches.", out.width="50%", fig.align='center'}

knitr::include_graphics("images/sp_richness_patches.png")
```


```{r, label = shannon, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Dung beetle Shannon diversity in three different --- and the forest matrix surrounding patches.", out.width="50%", fig.align='center'}

knitr::include_graphics("images/sp_shannon_patches.png")

```

```{r, label = simpson, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Dung beetle Simpson's index in three different --- and the forest matrix surrounding patches.", out.width="50%", fig.align='center'}
...
```{r bray-curtis, echo=FALSE,message = FALSE,warning=FALSE}
...
# bray<-read_csv("~/Rstudio/corridor_beetles/corridor_docs/eric_ms_thesis/tables/bray_patches.csv") 
bray<-read_csv(here("corridor_docs","eric_ms_thesis","tables","bray_patches.csv")) %>% kable(
      digits = 3,
      align="lcc",
      format="latex",
      row.names = FALSE,
      booktabs=T,
      linesep = "", #removes the blank line after every 5 lines
      caption = "Bray-Curtis dissimilarity comparing composition of dung beetles between patches.") %>%
  kable_styling(bootstrap_options = c("striped"),
                full_width = F,
                latex_options=c("scale_down","hold_position"),
                font_size = 12,
...
# aovM3<-read_csv("~/Rstudio/corridor_beetles/corridor_docs/eric_ms_thesis/tables/aovM3.csv") %>% 
  kable(
      ...
      booktabs=T,
      linesep = "", #removes the blank line after every 5 lines
      caption = "aovM3 patch type effects on...") %>%
  kable_styling(bootstrap_options = c("striped"),
                full_width = F,
                latex_options=c("scale_down","hold_position"),
                font_size = 12,
...
# M3<-read_csv("~/Rstudio/corridor_beetles/corridor_docs/eric_ms_thesis/tables/M3.csv") %>% 
  kable(
      ...
      booktabs=T,
      linesep = "", #removes the blank line after every 5 lines
      caption = "M3 patch type effects on...") %>%
  kable_styling(bootstrap_options = c("striped"),
                full_width = F,
                latex_options=c("scale_down","hold_position"),
                font_size = 12,
...
# M_shan<-read_csv("~/Rstudio/corridor_beetles/corridor_docs/eric_ms_thesis/tables/M_shan.csv") %>% 
  kable(
      ...
      booktabs=T,
      linesep = "", #removes the blank line after every 5 lines
      caption = "Shannon Diversity Model") %>%
  kable_styling(bootstrap_options = c("striped"),
                full_width = F,
                latex_options=c("scale_down","hold_position"),
                font_size = 12,
