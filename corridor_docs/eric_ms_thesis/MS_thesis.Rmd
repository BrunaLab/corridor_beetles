...
Differences in species richness showed no significance across patch types. The total number of species captured in each treatment varied by sampling block, however species totals generally ranged from 8 to 13 species. The patch with the most species captured was in the matrix at block 53n while the fewest were captured at the rectangle patch in block 8 (sp_richness.png). 
...
Values for Hill numbers also showed no significance between patch types. Modeled values for both Shannon and Simpson's diversity had relatively high variance but averaged out at approximately qD = 5.29 and qD = 4.07 respectively. 
