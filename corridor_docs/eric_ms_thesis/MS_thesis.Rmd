...
Statistical analysis of abundance focused on the six most abundant species. A generalized linear mixed model identified significant effects for both species ID and patch type on dung beetle abundance (Table 7). The baseline abundance (intercept) corresponds to the abundance of _Aphodius alloblackburneus_ in connected patches. Compared to this baseline, _Canthon vigilans_, _Ateuchus lecontei_, and _Phanaeus igneus_ showed significantly higher abundance  (β = 1.05, 0.73, 0.66 respectively; all p < 0.001), while _Onthophagus pennsylvanicus_ had lower abundance (β = –0.80, p < 0.001). 

Species-patch interactions indicated indicated species specific responses to treatments. In matrix plots, the positive effect on abundance was reduced for all other species (e.g., cvig × Matrix β = –0.59, p < 0.001). _Ateuchus lecontei_ and _Phanaeus igneus_ showed increased abundance in rectangle patches relative to expectations (β = 0.40 and 0.47, respectively; p < 0.05). Random effects indicated moderate variability among blocks (standard deviation of intercept = 0.51).  

### Species Richness

The number of species per patch varied from N = 8 (rectangle patch in block 8) to N = 13 (matrix patch in block 53N). Species richness was relatively even across patch types with matrix and connected patches averaging out to approximately N = 11 species and rectangle and winged patches averaging to approximately N = 10 species (Figure 4). Overall there were no significant effects, however species richness patterns were variable between sampling blocks. 


### Species Diversity

Values for Hill numbers showed no patterns for shifts in species composition by patch type. Data for Shannon diversity showed no differences in the distribution of rare species between patch types. On average connected patches had a value of D = 5.29, while mean estimates for all other patches remained close with +0.03% for rectangle, -0.02% for matrix, and -0.06% for winged patches. There was no significance between patch types but data was highly variable between patch types and sampling blocks (Figure 5). 

Values for Simpson Diversity also showed no differences but instead for more abundant species composing the dung beetle assemblages. Simpson diversity had a mean value of D = 4.07 in the connected patch, also with other values in close proximity with matrix +0.02%, rectangle +0.04%, and winged at -0.1%. There was no significant difference between patch types, however this data was also highly variable between patch type and sampling blocks (Figure 7). 
